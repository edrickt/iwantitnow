...
with open("info.txt") as r:
    infofile = r.read().splitlines()
...
class Email:
    ...
    def send_email(self, content, keyword, subreddit):
        try:
            ...
            server = smtplib.SMTP_SSL(infofile[0], infofile[1])
            server.login(infofile[2], infofile[3])
...
class Search:
    def __init__(self, email, subreddit, keyword):
        ...
        self.reddit = praw.Reddit(client_id=infofile[6],
                                  client_secret=infofile[7],
